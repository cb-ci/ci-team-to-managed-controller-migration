...
function checkControllerOnline () {
  ...
  while [ ! -n "$(curl  -IL  ${CONTROLLER_URL}/whoAmI/api/json?tree=authenticated | grep -o  'HTTP/2 200')" ]
  do
...
envsubst < ${CREATE_MM_TEMPLATE_YAML} > $GENDIR/${CONTROLLER_NAME}.yaml
envsubst < ${CREATE_MM_FOLDER_TEMPLATE_YAML} > $GENDIR/${CONTROLLER_NAME}-folder.yaml
...
# We apply the cjoc-controller-items.yaml to cjoc. Cjoc will create a new MC for us using our $GENDIR/${CONTROLLER_NAME}.yaml
echo "------------------  CREATING MANAGED CONTROLLER ------------------"
curl -XPOST \
   --user $TOKEN \
   "${CJOC_URL}/casc-items/create-items" \
    -H "Content-Type:text/yaml" \
   --data-binary @$GENDIR/${CONTROLLER_NAME}.yaml
...
curl -v  -XPOST \
    ...
    "${CONTROLLER_URL}/casc-items/create-items" \
     -H "Content-Type:text/yaml" \
    --data-binary @$GENDIR/${CONTROLLER_NAME}-folder.yaml -o $GENDIR/create-folder-output.log
...
mkdir -p $GENDIR/teams-${CONTROLLER_NAME}-jobs
#kubectl exec -it teams-${CONTROLLER_NAME}-0 --  tar -cvzf /tmp/${CONTROLLER_NAME}-job.tar.gz -C /var/jenkins_home/jobs/
kubectl cp teams-${CONTROLLER_NAME}-0:var/jenkins_home/jobs/${CONTROLLER_NAME}/jobs/ $GENDIR/teams-${CONTROLLER_NAME}-jobs/
kubectl cp $GENDIR/teams-${CONTROLLER_NAME}-jobs/. ${CONTROLLER_NAME}-0:var/jenkins_home/jobs/${CONTROLLER_NAME}/jobs
